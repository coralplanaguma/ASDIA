...
$$
\begin{align}
  \mathbb{E}[L(q({\bf x}, y)|{\bf x})] &= \mathbb{E}[-y \log q({\bf x}) - (1-y)\log (1-q({\bf x}))|x]\\
  &= -\log q({\bf x}) \mathbb{E[y]}-\log(1-q({\bf x}))(1 - \mathbb{E}[y])\\
  &=-p(y=1|{\bf x}) \log q({\bf x}) - (1 - p(y=1|{\bf x}))\log (1-q({\bf x}))\\
  &= -p(y=0|{\bf x}) \log (1-q({\bf x})) - p(y=1)\log (q({\bf x}))\\
  &= h(p_Y(.|{\bf x}), q_.({\bf x}))
\end{align}
$$
...
$$
\begin{align}
\mathbb{E}[L(q_{opt}({\bf x}), y)|x] &= h(p_Y(.|{\bf x}), q_.(x))\\
                                    &= h(q_{opt})

\end{align}
$$

Or, on amontré que $\mathbb{E}[L(q_{opt}({\bf x}), y)|x] \geq h(q_{opt})$. Donc d'après les résultats qui précèdent, $q_{opt}$ minimise l'espérance $\mathbb{E}[L(q_{opt}({\bf x}), y)]$
...
Voici le code R correspondant au modèle qu'on a choisi: ss
