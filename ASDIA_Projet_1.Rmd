...
for (i in 1:2){
  for (j in 1:2){
    for(k in 1:2){
      ...
      created_model<-create_model(nb_couches[i], nb_neurones[j], val_dropout[k])
      model = created_model
...
for (i in 0:length(wrong)){
  probas[i] = predict_proba(model, x_test)[wrong[i]]
}
...
* Dans quelles proportions les termes de valeur d'association $r^2$ supérieure à 0.03 apparaissent-ils dans les documents ? Représenter graphiquement ces proportions à l'aide d'un diagramme en barre. 


```{r eval = FALSE}
# Calculer la frequence des termes realisant la condition
diag_freq = function(corr){
  ind <- c()
...
# mots dans l'index et barplot
  ...
  b = barplot(sort(freq, decreasing = TRUE), col = "lightblue", las = 2)
  return(freq)
}
